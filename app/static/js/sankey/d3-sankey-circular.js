// https://github.com/tomshanley/d4-sankey-circular
// fork of https://github.com/d4/d4-sankey copyright Mike Bostock
;(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined'
      ? factory(
          exports,
          require('d4-array'),
          require('d4-collection'),
          require('d4-shape')
        )
      : typeof define === 'function' && define.amd
          ? define(['exports', 'd4-array', 'd4-collection', 'd4-shape'], factory)
          : factory(
              (global.d4 = global.d4 || {}),
              global.d4,
              global.d4,
  ...
  })(this, function (exports, d4Array, d4Collection, d4Shape) {
    'use strict'
    ...
    function center (node) {
      return node.targetLinks.length
        ...
        : node.sourceLinks.length
            ? d4Array.min(node.sourceLinks, targetDepth) - 1
            : 0
    }
    ...
    var sankey = function () {
      ...
      function computeNodeLinks (graph) {
        ...
        var nodeById = d4Collection.map(graph.nodes, id)
      ...
      function computeNodeValues (graph) {
        graph.nodes.forEach(function (node) {
          ...
          node.value = Math.max(
            d4Array.sum(node.sourceLinks, value),
            d4Array.sum(node.targetLinks, value)
          )
      ...
      function computeNodeBreadths (graph) {
        var columns = d4Collection
          .nest()
          .key(function (d) {
            return d.x0
          })
          .sortKeys(d4Array.ascending)
          .entries(graph.nodes)
          .map(function (d) {
            return d.values
          })
        ...
        function initializeNodeBreadth () {
          ...
          var ky = d4Array.min(columns, function (nodes) {
            return (y1 - y0 - (nodes.length - 1) * py) / d4Array.sum(nodes, value)
          })
            ...
            nodes.forEach(function (node) {
              ...
              if (node.sourceLinks.length || node.targetLinks.length) {
                if (node.partOfCycle /*&& n > 1 /* && depth != 0 */) {
                  ...
                  let avgTargetY = d4Array.mean(
                    node.sourceLinks,
                    linkTargetCenter
                  )
                  let avgSourceY = d4Array.mean(
                    node.targetLinks,
                    linkSourceCenter
                  )
    ...
    function addCircularPathData (graph) {
      ...
      let minY = d4.min(graph.links, function (link) { return link.source.y0 })
  ...
  var sankeyPath = function(link) {
    ...
    if (link.circular) {
      ...
      var normalPath = d4.linkHorizontal()
        .source(function (d) {
  ...
  function appendArrows(selection, arrowLength, gapLength, arrowHeadSize) {
          ...
          arrows.each(function (arrow) {
            ...
            let thisPath = d4.select(this).node();
            let parentG = d4.select(this.parentNode)
            ...
            let arrowHeadData = d4.range(numberOfArrows).map(function (d, i) {
